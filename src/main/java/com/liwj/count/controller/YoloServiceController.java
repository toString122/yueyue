...
public class YoloServiceController {
    ...
    public void markEveryDetectObject(Mat src, List<ObjectDetectionResult> results) {
        ...
        for (ObjectDetectionResult result : results) {
            ...
            log.info("class[{}]，p[{}%]，x[{}%]，y[{}%]，width[{}%]，height[{}%]", result.getClassName(), result.getConfidence() * 100f,x,y,width,height);
            // annotate on image
            rectangle(src,
    ...
    public double printTimeUsed(Mat src,String a) {
        // 总次数
        long totalNums = net.getPerfProfile(new DoublePointer());
        // 频率
        double freq = getTickFrequency() / 1000;
    ...
    @PostMapping("/")
    synchronized public  String saveImg(String pic,String word,String pwd,String key){
        if(key==null||key.equals("")){
            return "wu";
        }
        if(pwd==null|| !pwd.equals(configPwd)){
            return "wu";
        }
        ...
        pic = pic.replaceAll(" ", "+").split("base64,")[0];
        try
        {
            //Base64解码
        ...
        //识别
        // 读取文件到Mat
        Mat src = imread(uploadPath + "/" + fileName);
        ...
        double timeUsed = printTimeUsed(src, "");
        String finalStr ="";
        if(configType.equals("1") && (word!=null && !word.equals(""))&&word.length()==2){
            for (ObjectDetectionResult result : results) {
                if(result.getConfidence()>1){
                    continue;
                }
                if ( result.getClassName().equals(word.substring(0,1))){
                    log.info("class[{}]，p[{}%]，x[{}]，y[{}]，width[{}]，height[{}]，timeUsed[{}]", result.getClassName(), result.getConfidence() * 100f,
                            result.getX(),result.getY(),result.getWidth(),result.getHeight(),timeUsed);
                    finalStr = finalStr+result.getClassId()+","+result.getClassName()+","+result.getX()+","+result.getY()+","+result.getWidth()+","
                            +result.getHeight()+","+result.getConfidence() * 100f+"|";
                }
            ...
            for (ObjectDetectionResult result : results) {
                ...
                if (configType.equals("1") && result.getClassName().equals(word)){
                    log.info("class[{}]，p[{}%]，x[{}]，y[{}]，width[{}]，height[{}]，timeUsed[{}]", result.getClassName(), result.getConfidence() * 100f,
                            result.getX(),result.getY(),result.getWidth(),result.getHeight(),timeUsed);
                    finalStr = finalStr+result.getClassId()+","+result.getClassName()+","+result.getX()+","+result.getY()+","+result.getWidth()+","
                            +result.getHeight()+","+result.getConfidence() * 100f+"|";
                    break;
                }else if (configType.equals("0")){
                    log.info("class[{}]，p[{}%]，x[{}]，y[{}]，width[{}]，height[{}]，timeUsed[{}]", result.getClassName(), result.getConfidence() * 100f,
                            result.getX(),result.getY(),result.getWidth(),result.getHeight(),timeUsed);
                    finalStr = finalStr+result.getClassId()+","+result.getClassName()+","+result.getX()+","+result.getY()+","+result.getWidth()+","
                            +result.getHeight()+","+result.getConfidence() * 100f+"|";
        ...
        // 小于0.2保存图片
//        if (results.size()>0 && results.get(0).getConfidence()>0.2){
//            File file = new File(uploadPath + "/" + fileName);
//            file.delete();
//        }
        //失败返回CUO
...
//                }
        ...
        pointsManagementDaoImpl.consumePoint(key);
        return finalStr;
    ...
    public R upload_(@RequestParam("file") MultipartFile file, Map<String, Object> map) {
       ...
       // String url = saveMarkedImage(map, src);

        return R.ok().put("count",detectNum);
    }
     ...
     * @param src
     */
    public String saveMarkedImage(Map<String, Object> map, Mat src) {
        // 新的图片文件名称
        ...
        //上传腾讯云存储
        return cosUtils.upload(uploadPath + "/" + newFileName);
    }
}
